...
internal sealed class RandomStreamRecoveryTests
{
    ...
    public void StateAndIncrementRoundTripAndContinueTheSequence()
    {
        ...
        RandomStreamKey untouched = RandomStreamKey.Create(0x0500, 99UL);
        keys.Add(untouched);
        ...
        Expect.Multiple(() =>
        {
            ...
            // The artifact is canonical: ascending family key, then instance key. A recovery
            // artifact is reviewable text downstream, so its order cannot depend on the order a
            // run happened to instantiate its streams.
            for (int index = 1; index < artifact.Count; index++)
            {
                ...
                Assert.That(ascending, Is.True, "recovery records are ordered canonically");
            }
