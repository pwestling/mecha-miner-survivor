...
/// which is a reason to prefer it beyond its being the one the suite can run. Nothing here
/// says the evaluator is right about everything; a correction to it would be a validator
/// change inside the commit that establishes a baseline measured by that validator, so any
/// such finding is recorded and not applied in the same change.
/// </para>
/// <para>
...
internal sealed class SchemaCorpusDivergence
{
    ...
    internal string Render()
    {
        ...
        text.Append("# schemas and the content agree. See SchemaCorpusDivergence for the instrument\n");
        text.Append("# and for the two latent defects it is blind to by construction.\n");
        text.Append("#\n");
