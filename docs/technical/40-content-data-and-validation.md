...
- Where a contract value is defined relative to another quantity, the authored field names the quantity and its relative unit, and the compiler emits a derived companion in absolute units carrying this section's suffix: `primary_connector_width_in_mining_zone_diameters` is authored, `primary_connector_width_m` is derived. This `<quantity>_in_<reference-unit>` form — the case this bullet governs, distinct from the percentage and multiplier cases above — has exactly two instances in the tree today, both in the [standard map generation contract](../51-standard-map-generation-contract.md): the connector width named above and the deployment-clearance figure. Read this together with [Enemies and bosses](#enemies-and-bosses)' rule that derived geometry is never authored: that rule removes the absolute number from source, and this one says where it goes instead and what it is called.
