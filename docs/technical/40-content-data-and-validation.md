...
Four closed value vocabularies — `resource_class`, `persistence_class`, and `modifier_direction` on the resource definition, and `timestamp_provenance` on the encounter schedule — exist in `content/schemas/resource.schema.json` and `content/schemas/encounter-schedule.schema.json` respectively, and in the implementation's category tables, and no accepted document has ever granted any of them. This section grants them, on the standard [Minted content-ID grammars](#minted-content-id-grammars) above applies to prefixes: a token that has only ever appeared in a schema `enum`, a code comment, or a chat log carries no authority here, and being obvious was never a substitute for a grant. [Resources](#resources) below names two of these fields in prose while stating neither one's set, which is why naming a field there is not the same act as granting its vocabulary here.
