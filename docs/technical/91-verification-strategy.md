...
The vocabulary is open, and a kind is minted by adding it to the inventory below before it may appear in a registry file. The inventory as of this revision, including `compilation`, which is minted because the non-compilation rule below presupposes it and not because any entry was found reaching for it: `absent-value-assertion`, `artifact-sha256`, `assembly-metadata`, `assembly-sha256`, `benchmark-report-json`, `build-manifest-json`, `canonical-ordering`, `canonical-transcript`, `captured-artifact`, `changed-file-list`, `command-exit-code`, `command-output`, `compilation`, `diagnostic-run-record-json`, `drop-counters`, `engine-log`, `engine-report`, `expected-diagnostic-id`, `file-hash`, `frame-budget-table`, `golden-text`, `identity-equality`, `import-log`, `log-line`, `minimized-input-artifact`, `msbuild-property-values`, `pin-comparison`, `reference-capture`, `registry-report`, `rotated-file-set`, `runner-report-json`, `seed-identity`, `sink-touch-count`, `stdout-line`, `surviving-file-assertion`, `test-counts`, `test-output`, `text-diff`, `timeout-observation`, `verb-result-json`, `warning-count`. The inventory carries near-duplicates that a later revision should consolidate, among them `stdout-line` against `command-output` and the three hash kinds against one another; consolidating them edits registry files owned by their packages and is not done here.

`benchmark-report-json` names the `SCH-OBS-002` performance report in the [Component, Contract, and Schema Registry](./115-component-contract-and-schema-registry.md) § Schema registry, and not a file extension. That is what keeps the kind from over-claiming, and it is worth stating because the `-json` suffix invites the opposite reading. No document fixes the report's format: [Build, Dependencies, and Release Operations](./100-build-dependencies-and-release-operations.md) § Standard command surface has the `benchmark <id>` verb emit its canonical report and requires every verb to write structured evidence beneath `artifacts/`, without saying in what form; [Performance, Diagnostics, and Observability](./90-performance-diagnostics-and-observability.md) § Canonical benchmark scenarios and § Frame metrics fix what a benchmark report contains and § Structured logging fixes how long records are kept, and the only place a serialization is named is § Gameplay and balance metrics, for balance records rather than for benchmark reports. So the format is genuinely open, and a kind that names the schema stays true whichever serialization a benchmark ends up emitting, where a kind naming a file extension would assert a decision nobody has taken. This is also why the kind covers the `PERF-04` report: that report is an `SCH-OBS-002` document field by field — its scenario, device, and settings are pinned by § Canonical benchmark scenarios, the frame distributions and the subsystem counters it reports are the captures listed under § Frame metrics, and a tick-catch-up limit is a budget.
...
The field is misused in the registry in a single recognizable shape, which is two defects at once: a task or work-package ID where a verification ID belongs, and a `successor` on an entry whose status is not `retired`. What such an entry means is that a later task is expected to replace the verification, and that is not a successor, it is planned work. Record it in `deferredTo`, described in the section below, with the reason in the entry summary or the registry file's notes where a reader needs one, and set `successor` only when the entry is actually retired and its replacement has a `VER-*` ID.
...
### What `deferredTo` means

`deferredTo` names the work package or task that owns work the entry does not cover. Its target is a work-package or task identifier — `QUA-005` and `TASK-FND-009-001` are both well formed — and unlike `successor` it is legal on **any** status, `implemented` included. That is the point of the field rather than a contradiction in it: an entry is `implemented` for what it verifies, and something it deliberately does not verify, a budget in measured bytes or a device measurement or a scenario nobody has built, can be owed elsewhere at the same moment. The field adds an obligation held by another package and takes nothing back from the status, so an entry carrying it still means exactly what § What `evidenceKinds` means says its status means.

The two fields are now distinguishable by rule and not by judgment. `deferredTo` is deferral: other work, any status, a work-package or task ID. `successor` is replacement: `retired` only, a `VER-*` ID, and the decision procedure for it is the search immediately above rather than anything restated here. Every row that search returns which is not a retirement is an entry that wanted `deferredTo`. The mirror check reads the new field:

```
jq -r 'input_filename as $f | .entries[] | select(has("deferredTo")) | "\($f) \(.id) \(.status) -> \(.deferredTo)"' tests/verification/*.json
```

Any status is correct here, so the only defect a row can show is a `VER-*` target, which is a replacement wearing the wrong field name and belongs in `successor` under the rule above.

The field exists because its absence was doing damage, not because the vocabulary wanted another entry. Authors who needed to say “the work that would complete this belongs to another package” had no field for it and reached for `successor`, which was the nearest field rather than the right one. That produced rows wrong in both halves at once, and it went further than wrong rows: the two of them recorded in § Claim and measurement discipline below named `QUA-005`, a work-package ID, which a count-shaped claim this document had made about `successor` — that every entry carrying it named a task ID — could not classify at all, so the field's misuse and the claim's failure were one defect seen twice. A field whose name states its meaning is the fix. An author reaching for `successor` to express deferral was not being careless, they were choosing the only field on offer, and no amount of discipline about the old field supplies a missing one.

Neither rule is enforced yet, and both are meant to be. The registry validator required by § Validation and reporting below owes `deferredTo`'s target shape and `successor`'s retirement-only rule as gates rather than as searches someone has to remember to run; its implementation, `src/MechaMiner.Tools/Audit/RegistryValidator.cs`, is carried by the `FND-004` work and not by this document, so the obligation is recorded here and discharged there. Until it lands, the two searches in these sections are the whole of the check.
