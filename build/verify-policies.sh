...
section "VER-FND-001-011: Deterministic=true (the repository's own value)"
...
section "VER-FND-001-011 negative control: -p:Deterministic=false forced"
...
if [[ ! "${third_hash}" =~ ${SHA256_TEXT} || ! "${fourth_hash}" =~ ${SHA256_TEXT} ]]; then
  control_fail "VER-FND-001-011: negative-control build produced no hashable assembly; got '${third_hash}' and '${fourth_hash}'"
