...
This is a reading of an ambiguous sentence resolved by internal consistency with an unambiguous constraint in the same document, made under [Documentation Conventions § Change discipline](./conventions.md#change-discipline), which requires that contradictions are not left for readers to reconcile. It preserves behavior, so [doc 114's editorial-clarification allowance](../technical/114-autonomous-agent-execution-protocol.md#specification-maintenance-autonomy) supports it as well; that rule is scoped to technical documents and is secondary here, because this document is player-visible gameplay Markdown and outranks it. This is not a design decision handed down, it has no decision record, and an explicit ruling may override it.
