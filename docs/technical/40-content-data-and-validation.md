...
The recipe material pair is authored as `recipe_pair_material_ids`, and this section is what gives the field its `snake_case` name. Its two elements are resource IDs matching `^RSC-[0-9]{2}$`, never canonical letters: `docs/66:17` keeps the letters as "authoring shorthand", and the `RSC-` paragraph in [Minted content-ID grammars](#minted-content-id-grammars) establishes that a resource's ID and its `canonical_letter` are two fields with neither derived from the other, so a reference carrying only the letter would bind to the weaker of a resource's two identities. This **renames the field as it is currently authored** in `content/weapons/*.json` — today an object whose `material_codes` array holds the two letters — and **changes those element values from letters to IDs**. The rename and the `source_refs` scope prefix rename are **one commit**: the same files scope a source reference with `recipe_pair:`, and `content/schemas/README.md:88-90` makes a scope naming a field the definition does not have an `MMC-4003` build error, so renaming either one alone fails the build. That the name reached `content/schemas/weapon.schema.json` before it reached this document is the schema being right early rather than a schema defect being ratified here: a schema that mints a token the corpus has no use for is simply wrong, but this one requires the ID form because that is the only form over which the recipe-letter check can execute, which makes it load-bearing and owed the grant this paragraph supplies.
...
Those recipes are unordered and the authored array is not, which is one field carrying two orderings rather than a contradiction, and this is the single place that reconciles them. As a **recipe** the pair is an unordered set: fabrication does not depend on which material is written first, and every statement elsewhere in these documents that calls the pair unordered is making that claim and stays correct. As **authored data** the array is order-significant, because the concatenation of the two resources' canonical letters in the authored sequence is the weapon ID's own suffix; reordering the two elements denotes the same recipe and spells a different weapon ID. Both readings are enforced and neither enforcement is redundant: `content/schemas/weapon.schema.json:61` carries the distinction on the field itself, the catalog's distinctness check sorts each pair before deduplicating while its letter-spelling check does not sort at all, and `ARecipePairWrittenInTheOtherOrderFailsTheLetterSpelling` in `tests/MechaMiner.Content.Tests/Categories/CategorySemanticRuleTests.cs` fails if the letter comparison is ever made order-insensitive. Harmonising the two checks by sorting in both would delete the only check that catches a swapped pair, and delete it silently, because the sorted check keeps running and keeps passing.
