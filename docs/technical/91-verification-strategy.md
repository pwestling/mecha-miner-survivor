...
The second bullet's two values are in use and are defects to correct in the registry file that carries them, owned by that file's work package: `negative-control` and `tripwire-sentinel` both appear on `VER-FND-003-011` in `tests/verification/FND-003.json`, and that is the whole of it. They are excluded from the inventory below rather than grandfathered into it, which is what makes the correction owed rather than optional. The first bullet's path is an illustration and not a citation: no registry file in this repository carries a path-shaped kind, and `artifacts/architecture/architecture-forbidden-edges.txt` appears in no file except this sentence. The rule it states still binds the next author, but there is nothing to correct for it, and saying otherwise sends a reader looking for a defect that is not there.
...
None of the four is enforced by a gate. They constrain what an author may write, not what a runner may check.
