...
**The membership test.** An array is *semantically ordered* when some consumer reads its element positions: a check that indexes into it, a derived value computed from the sequence, or a documented reading that a reorder would falsify. The test is operational — reorder the elements and ask whether every consumer still agrees the definition is unchanged. If all do, the array is a set. If any does not, it is semantically ordered, and the consumer that disagrees is the evidence.

Two things are not evidence either way. `uniqueItems` constrains repetition, not order: a fixed sequence with no repeats satisfies it exactly as a set does. And a field's name is not a classification.

**For this section's purposes, a *stable ID* is a value matching one of the ID grammars this document declares.** A scope-prefixed per-field reference is not one: the prefix names the field the reference applies to and is not part of the ID. A closed vocabulary token is not one either: it is a term drawn from a set, not an identifier of an entity. Both are stipulated here because the term was previously usable both ways, and the alternative reading in each case — that the whole element is the ID — is available in the text and is not the one taken.

**Canonical ID order applies on either of two grounds: the elements are stable IDs as stipulated above, or a document states the array is unordered. No array meets the second ground today** — it is stated so that an array can be declared unordered by document rather than only by the shape of its elements, and its being currently unpopulated is deliberate rather than an omission. An array meeting neither ground, with no consumer reading its positions, is **unclassified** and keeps its authored order. Preserving asserts nothing; sorting asserts the array is a set.

**Examples, measured at `claude/hearth-thread-hrufl9-dat-006` `109ca57` — which is the sha the examples were verified at, not the sha of the commit that lands this text — and not an authority; classify by the test, not by this list.** `recipe_pair_material_ids`, because the concatenation of the two resources' canonical letters in authored order is the weapon ID's own suffix and a letter-spelling check reads it. `ore_upgradeable_stats`, because the contiguity check compares each row's `slot` to the array index — not because the rows carry a slot field, which alone would leave it a set. `minute_rows` and `ranks`, in both the PowerUp and Utility categories, on the same index-comparison ground. `modifier_application_order` is an example on **documentary grounds only**: its description's first sentence states the order in which modifier layers apply, which a reorder falsifies, while its coverage and cardinality are enforced by order-blind checks — and note that its sole authored occurrence is an object rather than an array, so the example concerns the declaration and not any authored instance.

**`source_refs` is unclassified.** By the stipulation above its elements are not stable IDs, so canonical ID order does not reach it; and the convention the corpus bears out — whole-definition sources first, per-field ones after — is asserted by no consumer, so it is not semantically ordered either. Authored order is preserved on the unclassified footing, and this section does not make the convention normative.

**This clause reaches an array irrespective of how its key is declared.** An array nested inside a parameter map, whose key comes from a registered descriptor rather than a field table, is governed by the same test as any other.
...
A vocabulary is **grounded** only when the document it cites **states the set**. A resolving citation is necessary and not sufficient. That rule is written out rather than assumed because this project already breaks it in both available ways: nine of the twenty-seven closed vocabularies the implementation declares cite a `doc_id` that no document in `docs/` carries — both figures measured at `eefdd71`, where the nine cite six distinct absent document IDs and the denominator fell from twenty-eight when the last artifact asserting a relic scope vocabulary was struck; the numerator is stated as measured at a sha rather than pinned by a test, so that drift in it shows up as visible staleness, and `resource_class` cites `TDD-CONTENT-DATA`, which is this document, which until this section did not mention the field at all. A reader who checked only that the citation resolved would have called that second case grounded. This section applies the rule to itself: every token below is either **transcribed** from prose quoted here by line, or **assigned here** and said to be, and wherever a token is assigned the search that failed to find a source is recorded, because "no source exists" and "no one looked" are indistinguishable on the page otherwise. Recording a failed search has one side effect worth naming once here rather than four times below: quoting the string that was not found puts that string into `docs/`, inside the very document reporting its absence. Every negative search below is therefore scoped to exclude this section's own record of it, so that an auditor who repeats the search finds exactly the hits named here and nothing the record manufactured for itself.
