...
`benchmark-report` is minted by this revision, and it joins the near-duplicate pairs named above rather than resolving them. It passes the test the bullets above state, because a benchmark scenario emits a canonical report and that report is a file a reader opens. It occurs on a single `registered` entry whose gate is not yet built, and that is correct use rather than an unused kind: on a `registered` entry the list is the evidence the verification will produce, so minting a kind before its artifact exists is exactly what `registered` is for, and the defect would be the unminted kind rather than the unbuilt gate. Two further values found in registry files are deliberately not minted. `diagnostic-counter` and `reference-identity` name a counter's observed value and object identity between two references, and both are measurements rather than forms of artifact; what a reader opens in either case is the test output, which is a kind already. `reference-identity` had itself replaced `allocation-count`, which is the shape this test is most useful against: a wrong value swapped for another value of the same wrong category, with nobody asking whether the field wanted an artifact form at all.
