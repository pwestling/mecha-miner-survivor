...
| 🚧 | Simulation core | Written, 252 tests passing in `MechaMiner.Simulation.Tests`, not on `master` — [#11](https://github.com/pwestling/mecha-miner-survivor/pull/11) **merged** on 11 Aug 2026, from head `claude/hearth-thread-3aamx2` into base `claude/hearth-thread-2vmaro-fnd-002`, which is not an ancestor of `master` |
| 🚧 | Playable slice | Movement only. [#19](https://github.com/pwestling/mecha-miner-survivor/pull/19) **merged** on 6 Aug 2026 — into `claude/hearth-thread-3aamx2` (merge commit `5f9e28c`), **not** into `master`. It now rides that branch instead of one of its own. [#11](https://github.com/pwestling/mecha-miner-survivor/pull/11) then **merged** on 11 Aug 2026, carrying it onto `claude/hearth-thread-2vmaro-fnd-002`, and as of 11 Aug 2026 two merges still separate it from `master`: [#3](https://github.com/pwestling/mecha-miner-survivor/pull/3), and a pull request nobody has opened yet. |
...
`claude/hearth-thread-3aamx2` ([#36](https://github.com/pwestling/mecha-miner-survivor/pull/36)) → `claude/hearth-thread-2vmaro-fnd-002` ([#3](https://github.com/pwestling/mecha-miner-survivor/pull/3)) → `claude/hearth-thread-2vmaro` → `master`

The playable slice ([#19](https://github.com/pwestling/mecha-miner-survivor/pull/19)) has already merged into `claude/hearth-thread-3aamx2`, so it rides that branch now instead of one of its own; that moved it one link along and did not put it on `master`. [#11](https://github.com/pwestling/mecha-miner-survivor/pull/11) has since merged too, on 11 Aug 2026, moving it a second link along onto `claude/hearth-thread-2vmaro-fnd-002` — which is not an ancestor of `master` either. The chain's first link is now [#36](https://github.com/pwestling/mecha-miner-survivor/pull/36), later work on `claude/hearth-thread-3aamx2` into the same base, which the slice is already past. Two pull requests have to land before the slice reaches `master`, and as of 11 Aug 2026 only one of them exists: #3, still a draft; the merge of `claude/hearth-thread-2vmaro` into `master` has no pull request open yet. The `./build.sh` command surface and CI both live on `claude/hearth-thread-2vmaro-fnd-002` and arrive on `master` in the same reconcile, which is why `master` currently has neither — it holds the specification, the content catalog, and an empty Godot shell. The last hop is pending work too, and the ancestry runs the opposite way from what this page used to say: as of 11 Aug 2026, `master` at `e17b8b6` is an ancestor of `claude/hearth-thread-2vmaro` (head `468d3c0`), not the reverse. The branch is 2 commits ahead of `master` and 0 behind. Those two commits are documentation-only, but they are not on `master`, so the branch needs a pull request of its own whether or not #3 has landed on it.
