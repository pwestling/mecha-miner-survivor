...
| `TASK-FND-003-001` | create pure NUnit projects, shared deterministic fixture utilities, and sample unit/property/golden tests | FND-001 Done | pure test projects/support | tests run without Godot, expose seed/reproduction on failure, and every test host proves its own reflection-disabled JSON switch is live: `AppContext.TryGetSwitch("System.Text.Json.JsonSerializer.IsReflectionEnabledByDefault", out var value)` true with `value` false, failing with a message that names the `JsonSerializerIsReflectionEnabledByDefault` MSBuild property and the file declaring it — a per-host `.csproj` or a shared props file such as `Directory.Build.props`, both of which bind because each host evaluates the property and emits its own `runtimeconfig.json`; only setting it on a plain library fails, since a library emits no `runtimeconfig.json` — rather than a bare boolean |
