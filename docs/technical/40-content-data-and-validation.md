...
Several of the prefixes below were agreed between working sessions in conversation before any document authorized them, and this section is what makes them real rather than a record that they already were: a prefix that has only ever appeared in a chat log, a code comment, or a schema `pattern` carries no authority here. That is the standard this project already applied to `common-ore` and `hyper-gold`, which carried slug IDs for as long as no accepted document had assigned them an ID token, and being obvious was never a substitute for one. They carry `RSC-07` and `RSC-08` now, minted in this section and adopted in `content/` by commit `4950f23`. The standard applies to us on the same terms.
...
`persistence_class` and `modifier_direction` are the field names this section **grants**, and the authored definition files carry neither of them. `content/resources/` holds `persistence`, which carries prose rather than a token — `A.json` has "Run-local; unspent units are lost when the run ends" — and the direction sits at `resonance_behavior.modifier.direction`. Both granted names now occur in `content/schemas/`, which arrived with the FND-002 merge, and `persistence_class` is on the mining site as well, at `content/schemas/mining-site.schema.json`, a category this section does not mention. Adopting them in the authored files is therefore a rename that has not happened, and this grant is not a description of `content/`: a reader who takes it as one goes looking for two fields that are not there.

**`timestamp_provenance` records whether a formation event's timestamps were read or rebuilt**, and it is granted last here because it belongs to the encounter schedule rather than the resource definition. Its two tokens are `authored` and `reconstructed`, and the field exists because one minute row's timestamps were reconstructed rather than transcribed: the four times minute 33's definition carries were derived rather than read out of the authored cell, and that derivation — not any particular wording of the cell — is what the field records. This sentence deliberately asserts nothing about how that cell reads on any given ref, because the provenance the field records is a fact about how these four values entered the content and does not change when the cell is reworded: a cell that stated four times outright would still not make these four transcribed from it. Whether the reconstruction is ultimately accepted as it stands or revised is not settled by this section, and the grant does not depend on which way it goes — under either outcome these four times entered the content by derivation rather than transcription, which is precisely the fact the field exists to carry. A consumer that reads those timestamps without reading this field is reading provisional numbers as accepted ones, and a bundle hash over that row is not evidence the times were authored. That reason is recorded here because until now it has lived only in `content/transcription-notes.md`, which is working notes rather than an accepted document — the notes keep the fuller history, the reconstruction's reasoning and the two ways it can later be discharged, and this document keeps the reason the field exists at all. A vocabulary whose rationale lives only in working notes is a vocabulary whose rationale is lost the first time those notes are archived. [Encounter schedule](#encounter-schedule) below opens with a list of the fields the schedule aggregate contains, and that list does not reach provenance, so naming the schedule's fields there is not the same act as granting this vocabulary here. That section does reach the field further down, in a sentence that points back to this one for the tokens instead of restating them.

Both tokens are **assigned here**. A pass over every `.md` file under `docs/` at commit `549af81` — **201** of them at that commit — found no document other than this section stating the set, and outside this section neither token appears in `docs/` there as one of a closed pair: `reconstructed` occurs **once** at that commit, at `docs/technical/10-runtime-architecture.md:78`, of presentation state that "may be discarded and reconstructed without changing the run result", and `authored` occurs throughout as ordinary English. Both figures — the file count and the single occurrence — are measurements at that one named commit and claim nothing beyond it: an auditor repeating this pass on a different ref, whether a later commit on this branch or any ref that merges another branch's added documents into it, **may** find different numbers, and should re-measure at `549af81` before concluding this record was wrong. Neither outcome is assumed: a ref that adds no documents under `docs/` can well reproduce both figures exactly, and a disagreement is only evidence about the corpus having moved once the measurement at `549af81` has been repeated and found to still hold. The commit is named precisely so that the negative finding can be repeated against the tree it was taken from; a count stated with no ref cannot be audited at all, which is the failure this section exists to avoid. The declaration's authority string is `TDD-CONTENT-DATA`, which is this document — so it **resolves**, and until this paragraph it did not mention the field. That is exactly the shape `resource_class` was in above, and the reason the rule at the top of this section is written as it is: the citation clears the first test and fails the one that matters, and nothing downstream could tell.
...
Two of the fields named above carry a closed vocabulary granted at [Minted value vocabularies](#minted-value-vocabularies): persistence class, and the modifier direction inside the resonance behavior registration. That section is where those two token sets and each token's provenance live, and naming either field here is not granting its vocabulary. It does not follow that every field named above has its vocabulary granted there, and the correspondence does not run either way. That section grants four vocabularies in total, and two of them are not items in the list above: `resource_class`, which the resource definition carries but this list omits, and `timestamp_provenance`, which belongs to the encounter schedule rather than to a resource. Conversely, inventory scope is named in the list above and is named in that section too, but only so that section can record that it deliberately grants no vocabulary for it.
...
The schedule also carries `timestamp_provenance`, which the field list above does not reach; that vocabulary is granted at [Minted value vocabularies](#minted-value-vocabularies), which is also where the reason the field exists is recorded.
